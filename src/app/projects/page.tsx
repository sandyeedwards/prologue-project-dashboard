...
export default async function ProjectsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const requestedMode = requestedAction === "compare" || requestedAction === "combine"
    ? requestedAction
    : one(params.mode);
  ...
  const comparedProjectGroups = mode === "compare" && reportProjects.length
    ? await getComparedProjectOperationalGroups(reportProjects)
    : [];
  const combinedGroups = mode === "combine" && reportProjects.length
    ? await getPortfolioOperationalGroups(reportProjects)
    : [];
  const combinedHistoricalSeries = mode === "combine" && reportProjects.length
    ? await getPortfolioHistoricalProfitSeries(reportProjects)
    : [];
  const pageTitle = mode === "combine" && reportProjects.length
    ? "Combined Project Report"
    : mode === "compare" && reportProjects.length
      ? "Compared Project Report"
      : "Compare or Combine Projects";
  const pageDescription = mode === "combine" && reportProjects.length
    ...
    <AppShell user={session.user} contentTone={mode === "combine" ? "portfolio" : "default"}>
      <main className="shell shell--wide projects-workspace executive-dashboard">
        ...
        {mode && reportProjects.length ? (
          mode === "compare" ? (
            <ProjectComparisonReport projects={reportProjects} projectGroups={comparedProjectGroups} />
          ) : (
            <CombinedPortfolioReport
        ...
        {compareOverLimit ? (
          <div className="selection-alert selection-alert--error" role="alert">
            ...
            <span>You selected {selected.length}. Remove {selected.length - 6} project{selected.length - 6 === 1 ? "" : "s"}, or use Combine selected for an unlimited aggregate report.</span>
          </div>
        ) : null}
        ...
        <section className="selection-action-bar selection-action-bar--top" aria-label="Project selection workspace">
          <div className="selection-action-bar__copy">
            <p className="eyebrow">Selection workspace</p>
            <strong>{selectedIds.length ? `${selectedIds.length} project${selectedIds.length === 1 ? "" : "s"} selected` : "Select projects below"}</strong>
            <span>Compare is limited to six projects. Combine has no application limit and aggregates the selected portfolio.</span>
            <span className="selection-action-bar__count"><b>{projects.length}</b> of {allProjects.length} reporting projects</span>
          </div>
          <div className="selection-action-bar__controls">
            <div className="selection-action-bar__buttons">
              <button className="button button--secondary" type="submit" form="project-selection" name="action" value="compare">
                Compare selected
                <small>Up to 6 projects</small>
              </button>
              <button className="button button--primary" type="submit" form="project-selection" name="action" value="combine">
                Combine selected
                <small>Unlimited projects</small>
              </button>
            </div>
            <span className="selection-action-bar__note">Filtered results remain available while comparison or combination reports are open.</span>
          </div>
        </section>

        <section className="projects-filter-region projects-filter-region--list" aria-label="Project search and filters">
          <ProjectFilterBar
            action="/projects"
            filter={filter}
            clients={clients}
        ...
        <form id="project-selection" className="project-selection-form" action="/projects" method="get">
          {hiddenFilterFields(filter).map(([name, value]) => (
            <input key={name} type="hidden" name={name} value={value} />
          ))}
          {mode ? <input type="hidden" name="mode" value={mode} /> : null}
          {selectedIds.filter((projectId) => !visibleProjectIds.has(projectId)).map((projectId) => (
          ...
          <div className="project-list-pagination" aria-label="Project list pagination">
            <div className="project-list-pagination__summary">
              <strong>{projects.length ? `Showing ${visibleFrom}–${visibleThrough}` : "Showing 0"}</strong>
              <span>of {projects.length} matching project{projects.length === 1 ? "" : "s"}</span>
            </div>

            <div className="project-list-pagination__pages">
              <button className="button button--secondary" type="submit" name="page" value={Math.max(1, currentPage - 1)} disabled={currentPage <= 1}>Previous</button>
              {pageButtons(currentPage, pageCount).map((page, index, pages) => (
                <span className="project-list-pagination__page-slot" key={page}>
                  {index > 0 && page - pages[index - 1] > 1 ? <span className="project-list-pagination__ellipsis" aria-hidden="true">…</span> : null}
                  <button
              ...
              <button className="button button--secondary" type="submit" name="page" value={Math.min(pageCount, currentPage + 1)} disabled={currentPage >= pageCount}>Next</button>
            </div>
          ...
          <div className="selection-action-bar selection-action-bar--bottom">
            <div>
              ...
              <span>Compare keeps each project separate. Combine produces one aggregate financial and performance view.</span>
            </div>
            <div className="selection-action-bar__buttons">
              <button className="button button--secondary" type="submit" name="action" value="compare">Compare selected</button>
              <button className="button button--primary" type="submit" name="action" value="combine">Combine selected</button>
            </div>
          </div>
        </form>
      </main>
