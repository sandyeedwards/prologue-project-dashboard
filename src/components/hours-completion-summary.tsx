...
export function HoursCompletionSummary({
  ...
  const normalized = rows
    .map((row) => {
      const estimated = finite(row.values.estimated) ? Math.max(row.values.estimated ?? 0, 0) : null;
      const logged = finite(row.values.logged) ? Math.max(row.values.logged ?? 0, 0) : null;
      const percent = estimated && estimated > 0 && logged !== null ? (logged / estimated) * 100 : null;
      const overrunPercent = percent !== null ? Math.max(percent - 100, 0) : 0;
      const tone =
        percent === null ? "unknown" : overrunPercent > 0 ? "over" : percent >= 90 ? "near" : "on-track";
      return {
        ...row,
        estimated,
        logged,
  ...
  const highestPercent = Math.max(
    100,
    ...normalized.map((row) => (row.percent !== null && Number.isFinite(row.percent) ? row.percent : 0)),
  );
  ...
  return (
    ...
    >
      ...
      <div className="hours-completion__rows">
        {normalized.map((row) => {
          const visiblePercent = row.percent === null ? 0 : Math.max(0, Math.min(row.percent, displayMaximum));
          ...
          const statusText =
            row.percent === null ? "No estimate" : row.percent > 100 ? "Over estimate" : "Of estimate";
      ...
      <p className="hours-completion__note">
        The chart uses one shared 1:1 percentage scale. The estimate marker is 100%; red extends proportionally beyond it when logged hours exceed the estimate.
      </p>
    </div>
  );
}
